...
+ (NSDate *)modificationDateAtPath:(NSString *)path {
    ...
    if (error) {
        NSAssert(FALSE, @"get attributes of item:%@, %@", path, error);
    } else {
...
+ (BOOL)setModificationDateAtPath:(NSString *)path date:(NSDate *)date {
    ...
    NSAssert(ok, @"failed set attributes:%@", error);
    return ok;
}
...
+ (NSArray *)getFileNameListAtPath:(NSString *)directoryPath extension:(NSString *)extension {
    ...
    if (error) {
        NSAssert(FALSE, @"error contentsOfDirectoryAtPath:%@ %@",directoryPath, error);
    } else {
...
+ (BOOL)removeItemAtPath:(NSString *)path {
    ...
    NSAssert(ret, @"error removeItemAtPath:%@, %@", path, error);
    return ret;
}
...
+ (BOOL)removeItemAtURL:(NSURL *)url {
    ...
    NSAssert(ret, @"error removeItemAtURL:%@, %@", [url absoluteString], error);
    return ret;
}
...
+ (BOOL)copyItemAtPath:(NSString *)path toPath:(NSString *)toPath {
    ...
    NSAssert(ret, @"error copyItemAtPath:%@, %@, %@", path, toPath, error);
    return ret;
}
...
+ (BOOL)copyItemAtURL:(NSURL *)url toURL:(NSURL *)toUrl {
    ...
    NSAssert(ret, @"error copyItemAtPath:%@, %@, %@", [url absoluteString], [toUrl absoluteString], error);
    return ret;
}
...
+ (BOOL)moveItemAtPath:(NSString *)path toPath:(NSString *)toPath {
    ...
    NSAssert(ret, @"error moveItemAtPath:%@, %@, %@", path, toPath, error);
    return ret;
}
...
+ (BOOL)moveItemAtURL:(NSURL *)url toURL:(NSURL *)toUrl {
    ...
    NSAssert(ret, @"error moveItemAtPath:%@, %@, %@", [url absoluteString], [toUrl absoluteString], error);
    return ret;
}
